...
本VRChatギミックはユーザーからの文字列と画像リンク及びゲーム内での撮影の入力(v2.0)をもらい、VRChatワールドで指定した書類ゲームオブジェクトに反映するギミックである。証明書のみならず、契約書あるいはテストペーパーなど、様々な入力が必要されている場面に使える。 最大32個の文字列入力欄と8個の画像入力欄が備えており、様々な場面にほぼ対応できる。 同期は入れておる。
アップデートしたい場合はできるだけ古いバージョンを削除してください。
This VRChat gimmick receives your text and image link input, and in-game phototaking and puts them on a game object. You can use it in situations that need input like making ID cards, contracts or test papers, etc., There are 32 text and 8 image URL blanks at maximum, I hope it can handle most situations. Global synced.
It would be better of you delete old version when you update.
这是一个接收用户的文字与图片链接，以及游戏内拍照输入，并将他们填入游戏物体的功能。可以用在填写各种证，合同以及试卷等需要输入与填写的场合上。 最多32个文字输入栏，8个图像链接输入栏。 全局同步。
更新时尽量删除老版本。
...
https://github.com/xianglong90II